...
(typically defined as `<auth-provider>`) which provides auth
services for web apps.
Any component within the body of `<auth-provider>` may observe
the current auth state and set up _effects_ to run should
that state change.
`<auth-provider>` also accepts messages to implement user sign-in and sign-out.
...
First, `import` `Auth` from `@unbndl` and `define` the custom element:
...
from `<auth-provider>` can access the state of the `Auth.Provider`.
